...
<body>
  ...
  <div class="container">
    <form class="add-car-form" action="{{ route('add-car') }}" method="POST">
        @csrf
        <h2>Add New Car</h2>
        @if(session('customer') && isset(session('customer')['id']))
        ...
        @enderror
        <input type="text" placeholder="Car Model" name="Car_Model" value="{{ old('car_model') }}">
        @error('Car_Model')
            <div class="error-message">{{ $message }}</div>
        @enderror
        ...
        @error('Car_Number')
            <div class="error-message">{{ $message }}</div>
        @enderror
        <textarea placeholder="Car Description" name="Car_Description">{{ old('car_description') }}</textarea>
